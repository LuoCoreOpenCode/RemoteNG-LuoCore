...
namespace mRemoteNG.UI.Controls
{
    ...
    public sealed class ConnectionContextMenu : ContextMenuStrip
    {
        ...
        public void SshTransferFile()
        {
            try
            {
                WindowsApp.Show(WindowType.SSHTransfer);
                WindowsApp.SshtransferForm.Hostname = _connectionTree.SelectedNode.Hostname;
                WindowsApp.SshtransferForm.Username = _connectionTree.SelectedNode.Username;
                WindowsApp.SshtransferForm.Password = _connectionTree.SelectedNode.Password;
                WindowsApp.SshtransferForm.Port = Convert.ToString(_connectionTree.SelectedNode.Port);
            }
            catch (Exception ex)
            {
                Runtime.MessageCollector.AddExceptionStackTrace(
        ...
        {
            WindowsApp.Show(WindowType.ActiveDirectoryImport);
        }

        private void OnImportPortScanClicked(object sender, EventArgs e)
        {
            WindowsApp.Show(WindowType.PortScan);
        }
