...
namespace mRemoteNG.UI.Controls.ConnectionTree
{
    ...
    public partial class ConnectionTree : TreeListView, IConnectionTree
    {
        ...
        private void AddNode(ConnectionInfo newNode)
        {
            ...
            _allowEdit = true;
            // 确保 SelectedItem 不为 null
            if (SelectedItem == null)
            {
                // 创建一个新的 ConnectionInfo 对象
                var newConnectionInfo = new ConnectionInfo();

                // 创建一个 OLVListItem 并将 ConnectionInfo 作为数据
                var newItem = new OLVListItem(newConnectionInfo);

                // 将新的 OLVListItem 添加到 Items 列表中
                Items.Add(newItem); // 确保将 newItem 添加到 Items 中

                // 设置 SelectedItem 为新的 OLVListItem
                SelectedItem = newItem;
            }
            if (SelectedItem != null && Items.IndexOf(SelectedItem) != -1)
            {
        ...
        private void tvConnections_AfterSelect(object sender, EventArgs e)
        {
            try
            {
                WindowsApp.ConfigForm.SelectedTreeNode = SelectedNode;
            }
            catch (Exception ex)
            {
